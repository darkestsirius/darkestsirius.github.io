...
建议在 namesilo 购买域名，完成后在 namesilo 官网进行域名解析，更详细的教程请自行Google
...
### 7.利用 GitHub 和 PicGo 搭建图床

Hugo 博客通过 markdown 语法写作，而在 markdown 中插入富文本（例如图片）是以链接的形式，对于图片这样的大文件，找一个可以通过链接访问、访问快捷的云存储方案无疑是更好的选择。为了简化图片上传流程，我们选择 PicGo 管理图片上传

你也可以选择其它图床网站，但建议选择在国内外均可访问、服务稳定、不会导致数据丢失、删库跑路的服务商

> ⚠️ 不要选择 Gitee！这货和度盘一个德行，说不定哪天你的图床就被吞了...

1. 在 Github 上创建一个新的仓库
2. 填写仓库先关资料，一般只需要选一个合适的仓库名，然后确保仓库为 `public` 其他的保持默认就好
3. 下载安装 [PicGo](https://github.com/Molunerfinn/PicGo/releases/)
4. 去 Github 创建一个 token，具体步骤：打开 `Settings -> Developer settings -> Personal access tokens`，最后点击 `generate new token`
5. `note` 处填写 `博客图床` ，并勾选 `repo` 相关权限